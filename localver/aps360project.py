...
class AutoencoderHalf(nn.Module):
    ...
    def __init__(self, start_outch=32, depth_ratio=16 / 9):
        ...
        self.name = "AEhalf_sod{0}_odr{1:.2f}".format(start_outch, depth_ratio)
...
class ImagesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        noisy_img = torchvio.read_image(self.paths[idx][0], torchvio.ImageReadMode.RGB).to(torch.float).to(Device)
        truth_img = torchvio.read_image(self.paths[idx][1], torchvio.ImageReadMode.RGB).to(torch.float).to(Device)
        noisy_img /= 255.
        truth_img /= 255.
...
train(model, model_dir, result_dir, train_ds, valid_ds, num_epochs=50)
